...
class Auth extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if(!empty($promo)){
            ...
            $data_promo['id_penawaran'] = $promo['id_penawaran'];
            $data_promo['token'] = $promo['token'];
        }
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('auth/login', $data_promo);
    ...
    function check_gender($array)
    {
    
        if(strtolower($array)  == 'male')
        { 
        return true;
        }
        else if(strtolower($array)  == 'female')
        {
        return true;
        }
        else
        {
        $this->form_validation->set_message('check_gender', 'You need to fill correct gender.');
        return false;
        }
    ...
    public function registration()
    {
        ...
        $data['title'] = 'BPR Amira User Regiatration';
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = [
                ...
                'date_birth' => strtotime($this->input->post('datebirth')),
                'image' => 'default.jpg',
            ...
            $this->db->insert('notifikasi', [
                'id_user' => '1',
                'judul' => 'Pendaftaran Nasabah',
                'url' => 'nasabah/tambahNasabah',
            ...
            Congratulation, Register successfull. <br> Please check your email regurally and activate your account!
            </div><br>');
            ...
            if ($user['is_active'] == 1) {
                ...
                if (password_verify($password, $user['password'])) {
                    ...
                    if ($data['role_id'] == '1') {
                        ...
                        if(!empty($id_penawaran) && !empty($token)){

                            $this->session->set_flashdata('promo',array('id_penawaran'=>$id_penawaran,'token'=>$token));
